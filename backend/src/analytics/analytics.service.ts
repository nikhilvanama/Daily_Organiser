...
export class AnalyticsService {
  ...
  async getSummary(userId: string, range: Range, todayLocal?: string) {
    ...
    const dailyActivity: Array<{ date: string; tasks: number; habits: number; journal: boolean }> = [];
    ...
    const byWeekday: Array<{ weekday: number; tasks: number }> = Array.from(
      { length: 7 }, (_, i) => ({ weekday: i, tasks: 0 }),
    );
