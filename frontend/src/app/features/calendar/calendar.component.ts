...
@Component({
    ...
    <div class="page animate-in">
      ...
      <div class="cal-grid card">
        ...
        @for (day of dayLabels; track day; let i = $index) {
          <div class="cal-label" [class.weekend-label]="(userProfile()?.weekendDays ?? ['Sat','Sun']).includes(day)">{{ day }}</div>
        }
  ...
  styles: [`
    ...
    .cal-cell.weekend.other-month { background: var(--weekend-bg-dim, #eaebee); }
...
export class CalendarComponent implements OnInit {
  ...
  private dayMap = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  ...
  isWeekendDay(date: Date): boolean {
    ...
    const weekends = profile?.weekendDays ?? ['Sat', 'Sun'];
    return weekends.includes(this.dayMap[date.getDay()]);
  }

  isBirthday(date: Date): boolean {
    const dob = this.userProfile()?.dateOfBirth;
    if (!dob) return false;
